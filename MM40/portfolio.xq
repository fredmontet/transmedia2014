...
let $fo :=  <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
     ...
     <fo:layout-master-set>
		<fo:simple-page-master master-name="coverPage" page-width="21cm" page-height="29.7cm">
			<fo:region-body background-color="white" margin-right="1cm" margin-left="1cm" margin-top="1cm"/>
	...
	<fo:page-sequence master-reference="coverPage" font-family="Helvetica">
		<fo:flow flow-name="xsl-region-body">
			<fo:block background-image="http://www.olfraime.com/testTrans/backgroundCover_portfolio.svg" background-repeat="no-repeat">
				...
				<fo:block text-align="right" margin-right="0cm" margin-top="7.5cm" font-size="12pt" color="rgb(0,0,0)">
	...
	<fo:page-sequence master-reference="resumePage" font-family="Helvetica">
		<fo:flow flow-name="xsl-region-body">
			<fo:block>
	...
	return
	<fo:page-sequence master-reference="projectPage" font-family="Helvetica">
		<fo:flow flow-name="xsl-region-body">
			<fo:block>
				<fo:block text-align="center" margin-bottom="0.5cm">
				{if ($projet[@id eq "eb005e-2014"]) then
					<fo:external-graphic  src="{concat($servRoot,$projet/TM:image/@url)}"   width="15.075cm"  content-width="19cm"  scaling="uniform" block-progression-dimension="14cm"/>
				else 
					<fo:external-graphic  src="{concat($servRoot,$projet/TM:image/@url)}"   width="15.075cm"  content-width="19cm"  scaling="uniform"/>
					}
				</fo:block>
				<fo:block background-color="white">
					<fo:block font-size="25pt" color="rgb(0,0,0)" margin-top="0.5cm">{$projet/TM:nom/text()}</fo:block>
				...
				{for $dom in $projet/TM:domaine
				let $rose:= concat($servRoot,"images/niveauRose.svg")
				let $gris:= concat($servRoot,"images/niveauNoir.svg")
				let $numRose:= $dom/@poids cast as xs:integer
				let $numGris:=5-$numRose
				return
			<fo:table margin-top="0.3cm">
				<fo:table-body>
